...
from vex.dom.views import NodeType, SimplifiedNode
from vex.llm.messages import ContentPartImageParam, ContentPartTextParam, ImageURL, SystemMessage, UserMessage
from vex.observability import observe_debug
from vex.utils import is_new_tab_page, sanitize_surrogates

if TYPE_CHECKING:
	from vex.agent.views import AgentStepInfo
...
class SystemPrompt:
	def __init__(
		...
		self.is_vex_model = is_vex_model
	...
	def _load_prompt_template(self) -> None:
		...
		try:
			...
			if self.is_vex_model:
				if self.flash_mode:
					template_filename = 'system_prompt_vex_flash.md'
				elif self.use_thinking:
					template_filename = 'system_prompt_vex.md'
				else:
					template_filename = 'system_prompt_vex_no_thinking.md'
			...
			with (
				importlib.resources.files('vex.agent.system_prompts')
