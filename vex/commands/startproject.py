...
from vex.utils.template import render_templatefile, string_camelcase
...
class Command(ScrapyCommand):
    ...
    def _copytree(self, src: Path, dst: Path) -> None:
        ...
        More info at:
        https://github.com/vex/vex/pull/2005
        """
        ignore = IGNORE
    ...
    def run(self, args: list[str], opts: argparse.Namespace) -> None:
        ...
        if (project_dir / "vex.cfg").exists():
            self.exitcode = 1
            print(f"Error: vex.cfg already exists in {project_dir.resolve()}")
            return
        ...
        print("    vex genspider example example.com")
    ...
    def templates_dir(self) -> str:
        ...
        return str(
            Path(
                self.settings["TEMPLATES_DIR"] or Path(vex.__path__[0], "templates"),
                "project",
            )
        )
