...
def collect_vex_settings_refs(app: Sphinx, doctree: document) -> None:
    env = app.builder.env

    if not hasattr(env, "vex_all_settings"):
        emptyList: list[SettingData] = []
        env.vex_all_settings = emptyList  # type: ignore[attr-defined]

    for node in doctree.findall(is_setting_index):
        setting_name, refid = get_setting_name_and_refid(node)

        env.vex_all_settings.append(  # type: ignore[attr-defined]
            SettingData(
                docname=env.docname,
    ...
    for node in doctree.findall(SettingslistNode):
        ...
        settings_list.extend(
            [
                ...
                for d in sorted(env.vex_all_settings, key=itemgetter("setting_name"))  # type: ignore[attr-defined]
...
def source_role(
    ...
    ref = "https://github.com/vex/vex/blob/master/" + text
...
def issue_role(
    ...
    ref = "https://github.com/vex/vex/issues/" + text
...
def commit_role(
    ...
    ref = "https://github.com/vex/vex/commit/" + text
...
def rev_role(
    ...
    ref = "http://hg.vex.org/vex/changeset/" + text
...
def setup(app: Sphinx) -> None:
    ...
    app.connect("doctree-read", collect_vex_settings_refs)
