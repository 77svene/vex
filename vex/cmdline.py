...
import vex
from vex.commands import BaseRunSpiderCommand, ScrapyCommand, ScrapyHelpFormatter
from vex.crawler import AsyncCrawlerProcess, CrawlerProcess
from vex.exceptions import UsageError
from vex.utils.misc import walk_modules
from vex.utils.project import get_project_settings, inside_project
from vex.utils.python import garbage_collect
from vex.utils.reactor import _asyncio_reactor_path
...
def _iter_command_classes(module_name: str) -> Iterable[type[ScrapyCommand]]:
    ...
    # vex.utils.spider.iter_spider_classes
...
def _get_commands_from_entry_points(
    inproject: bool, group: str = "vex.commands"
...
def _get_commands_dict(
    ...
    cmds = _get_commands_from_module("vex.commands", inproject)
...
def _print_commands(settings: BaseSettings, inproject: bool) -> None:
    ...
    print("Usage:")
    print("  vex <command> [options] [args]\n")
    ...
    print()
    print('Use "vex <command> -h" to see more info about a command')
...
def _print_unknown_command(
    ...
    print('Use "vex" to see available commands')
...
def execute(argv: list[str] | None = None, settings: Settings | None = None) -> None:
    ...
    parser = ScrapyArgumentParser(
        ...
        usage=f"vex {cmdname} {cmd.syntax()}",
...
def _run_command_profiled(
    ...
    if opts.profile:
        sys.stderr.write(f"vex: writing cProfile stats to {opts.profile!r}\n")
    loc = locals()
    p = cProfile.Profile()
