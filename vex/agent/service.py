...
if TYPE_CHECKING:
	from vex.skills.views import Skill

from dotenv import load_dotenv

from vex.agent.cloud_events import (
...
from vex.agent.message_manager.utils import save_conversation
from vex.llm.base import BaseChatModel
from vex.llm.exceptions import ModelProviderError, ModelRateLimitError
from vex.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from vex.tokens.service import TokenCost
...
from vex import Browser, BrowserProfile, BrowserSession
from vex.agent.judge import construct_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from vex.agent.gif import create_history_gif
from vex.agent.message_manager.service import (
	MessageManager,
)
...
from vex.browser.events import _get_timeout
from vex.browser.session import DEFAULT_BROWSER_PROFILE
from vex.browser.views import BrowserStateSummary
from vex.config import CONFIG
from vex.dom.views import DOMInteractedElement, MatchLevel
from vex.filesystem.file_system import FileSystem
from vex.observability import observe, observe_debug
from vex.telemetry.service import ProductTelemetry
from vex.telemetry.views import AgentTelemetryEvent
from vex.tools.registry.views import ActionModel
from vex.tools.service import Tools
from vex.utils import (
	URL_PATTERN,
	_log_pretty_path,
	check_latest_vex_version,
	get_vex_version,
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if llm is None:
			...
			if default_llm_name:
				from vex.llm.models import get_llm_by_name

				llm = get_llm_by_name(default_llm_name)
			else:
				# No default LLM specified, use the original default
				from vex import ChatBrowserUse
		...
		supports_coordinate_clicking = any(
			pattern in model_name for pattern in ['claude-sonnet-4', 'claude-opus-4', 'gemini-3-pro', 'vex/']
		)
		...
		if skill_service is not None:
			...
			from vex.skills import SkillService
		...
		self.agent_directory = base_tmp / f'vex_agent_{self.id}_{timestamp}'
		...
		self._set_vex_version_and_source(source)
		...
		# Check if model is a vex fine-tuned model (uses simplified prompts)
		is_vex_model = 'vex/' in self.llm.model.lower()
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'vex.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from vex.screenshots.service import ScreenshotService
	...
	def _set_vex_version_and_source(self, source_override: str | None = None) -> None:
		"""Get the version from pyproject.toml and determine the source of the vex package"""
		# Use the helper function for version detection
		version = get_vex_version()
	...
	async def _judge_trace(self) -> JudgementResult | None:
		...
		if self.judge_llm.provider == 'vex':
		...
		if self.history.history[-1].result[-1].is_done:
			...
			if judgement:
				...
				if judgement.reached_captcha:
					...
					judge_log += '   👉 🥷 Use Browser Use Cloud for the most stealth browser infra: https://docs.vex.com/customize/browser/remote\n'
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from vex.llm.messages import AssistantMessage, UserMessage
	...
	async def _log_agent_run(self) -> None:
		...
		if CONFIG.BROWSER_USE_VERSION_CHECK:
			latest_version = await check_latest_vex_version()
			if latest_version and latest_version != self.version:
				self.logger.info(
					f'📦 Newer version available: {latest_version} (current: {self.version}). Upgrade with: uv add vex=={latest_version}'
				)
	...
	def _log_first_step_startup(self) -> None:
		...
		if len(self.history.history) == 0:
			self.logger.info(
				f'Starting a vex agent with version {self.version}, with provider={self.llm.provider} and model={self.llm.model}'
			)
	...
	def _log_final_outcome_messages(self) -> None:
		...
		if is_successful is False or is_successful is None:
			...
			self.logger.info('   Open a short issue on GitHub: https://github.com/vex/vex/issues')
	...
	async def run(
		...
		from vex.utils import SignalHandler
	...
	async def _generate_rerun_summary(
		...
		from vex.agent.views import RerunSummaryAction
		...
		from vex.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
		...
		try:
			...
			from vex.llm.messages import BaseMessage
	...
	async def _execute_ai_step(
		...
		"""
		from vex.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from vex.llm.messages import SystemMessage, UserMessage
		from vex.utils import sanitize_surrogates
		...
		try:
			from vex.dom.markdown_extractor import extract_clean_markdown
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from vex.agent.variable_detector import detect_variables_in_history
