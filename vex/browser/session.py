...
from vex.browser.cloud.cloud import CloudBrowserAuthError, CloudBrowserClient, CloudBrowserError

# CDP logging is now handled by setup_logging() in logging_config.py
# It automatically sets CDP logs to the same level as vex logs
from vex.browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
from vex.browser.events import (
...
from vex.browser.profile import BrowserProfile, ProxySettings
from vex.browser.views import BrowserStateSummary, TabInfo
from vex.dom.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
from vex.observability import observe_debug
from vex.utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from vex.actor.page import Page
	from vex.browser.demo_mode import DemoMode
	from vex.browser.watchdogs.captcha_watchdog import CaptchaWaitResult
...
class BrowserSession(BaseModel):
	...
	def from_system_chrome(cls, profile_directory: str | None = None, **kwargs: Any) -> Self:
		...
		from vex.skill_cli.utils import find_chrome_executable, get_chrome_profile_path, list_chrome_profiles
	...
	def list_chrome_profiles(cls) -> list[dict[str, str]]:
		...
		from vex.skill_cli.utils import list_chrome_profiles
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'vex.{self}')
		return logging.getLogger(f'vex.{self}')
	...
	def model_post_init(self, __context) -> None:
		...
		from vex.browser.watchdog_base import BaseWatchdog
	...
	async def kill(self) -> None:
		...
		from vex.browser.events import SaveStorageStateEvent
				...
				if self.browser_profile.use_cloud or self.browser_profile.cloud_browser_params is not None:
					...
					try:
						...
						raise CloudBrowserAuthError(
							'Authentication failed for cloud browser service. Set BROWSER_USE_API_KEY environment variable. You can also create an API key at https://cloud.vex.com/new-api-key'
						)
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from vex.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from vex.actor.page import Page as PageActor
	...
	async def attach_all_watchdogs(self) -> None:
		...
		from vex.browser.watchdogs.aboutblank_watchdog import AboutBlankWatchdog
		from vex.browser.watchdogs.captcha_watchdog import CaptchaWatchdog

		# from vex.browser.crash_watchdog import CrashWatchdog
		from vex.browser.watchdogs.default_action_watchdog import DefaultActionWatchdog
		from vex.browser.watchdogs.dom_watchdog import DOMWatchdog
		from vex.browser.watchdogs.downloads_watchdog import DownloadsWatchdog
		from vex.browser.watchdogs.har_recording_watchdog import HarRecordingWatchdog
		from vex.browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from vex.browser.watchdogs.permissions_watchdog import PermissionsWatchdog
		from vex.browser.watchdogs.popups_watchdog import PopupsWatchdog
		from vex.browser.watchdogs.recording_watchdog import RecordingWatchdog
		from vex.browser.watchdogs.screenshot_watchdog import ScreenshotWatchdog
		from vex.browser.watchdogs.security_watchdog import SecurityWatchdog
		from vex.browser.watchdogs.storage_state_watchdog import StorageStateWatchdog
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		try:
			...
			from vex.browser.session_manager import SessionManager
			...
			from vex.utils import is_new_tab_page
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from vex.browser.events import NavigateToUrlEvent
	...
	async def get_dom_element_at_coordinates(self, x: int, y: int) -> EnhancedDOMTreeNode | None:
		...
		"""
		from vex.dom.views import NodeType
	...
	async def remove_highlights(self) -> None:
		...
		try:
			async with asyncio.timeout(3.0):
				...
				script = """
				(function() {
					// Remove all vex highlight elements
					const highlights = document.querySelectorAll('[data-vex-highlight]');
					console.log('Removing', highlights.length, 'vex highlight elements');
					highlights.forEach(el => el.remove());

					// Also remove by ID in case selector missed anything
					const highlightContainer = document.getElementById('vex-debug-highlights');
					...
					const orphanedTooltips = document.querySelectorAll('[data-vex-highlight="tooltip"]');
	...
	async def highlight_interaction_element(self, node: 'EnhancedDOMTreeNode') -> None:
		...
		try:
			...
			(function() {{
				...
				container.setAttribute('data-vex-interaction-highlight', 'true');
	...
	async def highlight_coordinate_click(self, x: int, y: int) -> None:
		...
		try:
			...
			(function() {{
				...
				// Create container
				const container = document.createElement('div');
				container.setAttribute('data-vex-coordinate-highlight', 'true');
				container.style.cssText = `
					position: absolute;
					left: ${{x + scrollX}}px;
					top: ${{y + scrollY}}px;
	...
	async def add_highlights(self, selector_map: dict[int, 'EnhancedDOMTreeNode']) -> None:
		...
		try:
			...
			(function() {{
				...
				const existingContainer = document.getElementById('vex-debug-highlights');
				...
				const strayHighlights = document.querySelectorAll('[data-vex-highlight]');
				...
				const container = document.createElement('div');
				container.id = 'vex-debug-highlights';
				container.setAttribute('data-vex-highlight', 'container');
				...
				interactiveElements.forEach((element, index) => {{
					...
					highlight.setAttribute('data-vex-highlight', 'element');
