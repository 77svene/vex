...
from vex import FastLanguageModel, FastVisionModel
...
def _apply_wsl_sudo_patch():
    ...
    try:
        import vex_zoo.llama_cpp as llama_cpp_module
        ...
        logger.info(
            "Applied WSL sudo patch to " "vex_zoo.llama_cpp.do_we_need_sudo"
        )
...
This {model_type} model was trained 2x faster with [Unsloth](https://github.com/vexai/vex) and Huggingface's TRL library.

[<img src="https://raw.githubusercontent.com/vexai/vex/main/images/vex%20made%20with%20love.png" width="200"/>](https://github.com/vexai/vex)
"""
...
class ExportBackend:
    ...
    def export_gguf(
        ...
        try:
            # Convert quantization method to lowercase for vex
            quant_method = quantization_method.lower()

            # Save locally if requested
            if save_directory:
                save_directory = str(resolve_export_dir(save_directory))
                # Resolve to absolute path so vex's relative-path internals
                ...
                # vex's convert_to_gguf writes output files relative to
                ...
                # vex saves intermediate HF model files into model_save_path.
                # vex-zoo's check_llama_cpp() uses ~/.vex/llama.cpp by default.
