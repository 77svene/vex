...
_DEMO_PANEL_SCRIPT = r"""(function () {
  ...
  const EXCLUDE_ATTR = 'data-vex-exclude-' + SESSION_ID;
  const PANEL_ID = 'vex-demo-panel';
  const STYLE_ID = 'vex-demo-panel-style';
  ...
  const TOGGLE_BUTTON_ID = 'vex-demo-toggle';
  ...
  function initializePanel() {
    ...
    if (savedWidth) {
      document.documentElement.style.setProperty('--vex-demo-panel-width', `${savedWidth}px`);
    }
  ...
  function addStyles() {
    ...
    style.textContent = `
      #${PANEL_ID} {
        ...
        top: 0;
        right: 0;
        width: var(--vex-demo-panel-width, 340px);
      ...
      #${PANEL_ID} .vex-logo img {
        height: 36px;
      }

      #${PANEL_ID} .vex-header-actions {
        margin-left: auto;
        display: flex;
        align-items: center;
        gap: 8px;
      ...
      #${PANEL_ID} .vex-close-btn:hover {
        background: rgba(255, 255, 255, 0.15);
        border-color: rgba(255, 255, 255, 0.35);
      }

      #${PANEL_ID} .vex-demo-body {
        flex: 1;
        overflow-y: auto;
      ...
      #${PANEL_ID} .vex-demo-body::-webkit-scrollbar {
        width: 8px;
      }

      #${PANEL_ID} .vex-demo-body::-webkit-scrollbar-thumb {
      ...
      .vex-demo-entry {
        ...
        animation: vex-fade-in 0.25s ease;
        background: #000000;
      }

      .vex-demo-entry:last-child {
      ...
      .vex-markdown-content {
        margin: 0;
        line-height: 1.5;
      }

      .vex-markdown-content p {
        margin: 0 0 8px 0;
      }

      .vex-markdown-content p:last-child {
        margin-bottom: 0;
      }

      .vex-markdown-content h1,
      .vex-markdown-content h2,
      .vex-markdown-content h3 {
        margin: 8px 0 4px 0;
        font-weight: 600;
        color: #f8f9ff;
      }
      ...
      .vex-markdown-content ul,
      .vex-markdown-content ol {
        margin: 4px 0 4px 16px;
        padding: 0;
      }

      .vex-markdown-content li {
        margin: 2px 0;
      }

      .vex-markdown-content a {
        color: #60a5fa;
        text-decoration: underline;
      }

      .vex-markdown-content a:hover {
        color: #93c5fd;
      }

      .vex-markdown-content strong {
        font-weight: 600;
        color: #f8f9ff;
      }

      .vex-markdown-content em {
        font-style: italic;
      }

      .vex-demo-entry:not(.expanded) .vex-markdown-content {
        max-height: 120px;
        overflow: hidden;
      ...
      .vex-demo-entry.level-info { border-left-color: #60a5fa; }
      .vex-demo-entry.level-action { border-left-color: #34d399; }
      .vex-demo-entry.level-thought { border-left-color: #f97316; }
      .vex-demo-entry.level-warning { border-left-color: #fbbf24; }
      .vex-demo-entry.level-success { border-left-color: #22c55e; }
      .vex-demo-entry.level-error { border-left-color: #f87171; }
  ...
  function buildPanel() {
    ...
    header.className = 'vex-demo-header';
    const title = document.createElement('div');
    title.className = 'vex-logo';
    const logo = document.createElement('img');
    logo.src = 'https://raw.githubusercontent.com/vex/vex/main/static/vex-dark.png';
    logo.alt = 'Browser-use';
    logo.loading = 'lazy';
    title.appendChild(logo);
    const actions = document.createElement('div');
    actions.className = 'vex-header-actions';
    const closeBtn = document.createElement('button');
    closeBtn.type = 'button';
    closeBtn.className = 'vex-close-btn';
  ...
  function adjustLayout() {
    ...
    document.documentElement.style.setProperty('--vex-demo-panel-width', `${width}px`);
    if (state.isOpen) {
  ...
  function hydrateFromStoredMarkup() {
    ...
    try {
      ...
      if (html) {
        ...
        for (const entryNode of state.list.querySelectorAll('.vex-demo-entry')) {
          const toggle = entryNode.querySelector('.vex-entry-toggle');
          if (toggle) {
  ...
  function createEntryNode(entry) {
    ...
    if (messageText.length > 160) {
      ...
      toggle.type = 'button';
      toggle.className = 'vex-entry-toggle';
  ...
  function applyPersistedExpansion(node) {
    ...
    try {
      ...
      if (id && expanded.has(id)) {
        ...
        const toggle = node.querySelector('.vex-entry-toggle');
        if (toggle) {
  ...
  window.addEventListener('vex-log', handleLogEvent);
})();
"""
...
(() => {{
	...
	const event = new CustomEvent('vex-log', {{ detail }});
