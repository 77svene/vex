...
if len(sys.argv) > 1 and sys.argv[1] == 'install':
	...
	if result.returncode == 0:
		...
		print('🚀 Ready to use! Run: uvx vex')
	else:
		print('\n❌ Installation failed')
		sys.exit(1)
	sys.exit(0)
...
if 'init' in sys.argv:
	from vex.init_cmd import INIT_TEMPLATES
	from vex.init_cmd import main as init_main
...
if '--template' in sys.argv:
	...
	output_path = Path(output) if output else Path.cwd() / f'vex_{template}.py'
	...
	try:
		...
		click.echo('\nNext steps:')
		click.echo('  1. Install vex:')
		click.echo('     uv pip install vex')
		click.echo('  2. Set up your API key in .env file or environment:')
		click.echo('     BROWSER_USE_API_KEY=your-key')
		click.echo('     (Get your key at https://cloud.vex.com/new-api-key)')
...
from vex import Agent, Controller
from vex.agent.views import AgentSettings
from vex.browser import BrowserProfile, BrowserSession
from vex.logging_config import addLoggingLevel
from vex.telemetry import CLITelemetryEvent, ProductTelemetry
from vex.utils import get_vex_version
...
try:
	...
	print(
		'⚠️ CLI addon is not installed. Please install it with: `pip install "vex[cli]"` and try again.', file=sys.stderr
	)
	sys.exit(1)
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('vex.'):
		...
		# Configure vex logger and all its sub-loggers
		vex_logger = logging.getLogger('vex')
		vex_logger.propagate = False  # Don't propagate to root logger
		vex_logger.handlers = [log_handler]  # Replace any existing handlers
		vex_logger.setLevel(root.level)

		# Also ensure agent loggers go to the main output
		# Use a wildcard pattern to catch all agent-related loggers
		for logger_name in ['vex.Agent', 'vex.controller', 'vex.agent', 'vex.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'vex.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('vex.on_mount')
	...
	def run_task(self, task: str) -> None:
		...
		# Get the logger
		logger = logging.getLogger('vex.app')
	...
	async def action_quit(self) -> None:
		...
		self.exit()
		print('\nTry running tasks on our cloud: https://vex.com')
		...
		with Container(id='main-container'):
			...
			with Container(id='links-panel'):
				with HorizontalGroup(classes='link-row'):
					...
					yield Link('https://vex.com', url='https://vex.com', classes='link-white link-url')
				...
				with HorizontalGroup(classes='link-row'):
					...
					yield Link(
						'https://github.com/vex/awesome-prompts',
						url='https://github.com/vex/awesome-prompts',
				...
				with HorizontalGroup(classes='link-row'):
					...
					yield Link(
						'https://github.com/vex/vex/issues',
						url='https://github.com/vex/vex/issues',
...
async def run_prompt_mode(prompt: str, ctx: click.Context, debug: bool = False):
	"""Run vex in non-interactive mode with a single prompt."""
...
async def textual_interface(config: dict[str, Any]):
	...
	# Prevent vex from setting up logging at import time
	os.environ['BROWSER_USE_SETUP_LOGGING'] = 'false'

	logger = logging.getLogger('vex.startup')
	...
	try:
		...
		try:
			from vex.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	from vex.sync.auth import DeviceAuthClient
	...
	try:
		...
		from vex.sync.service import CloudSync
		...
		task_event = CreateAgentTaskEvent(
			...
			llm_model='auth-flow',
			task='🔐 Complete authentication and join the vex community',
			user_id=auth_client.temp_user_id,
			device_id=auth_client.device_id,
			done_output=None,
		...
		try:
			...
			from vex.utils import create_task_with_error_handling
		...
		if success:
				...
				actions=[
					{
						...
						'done': {
							'success': True,
							'text': '⭐ Starred vex/vex repository! Welcome to the community!',
						},
					}
				],
				next_goal='⭐ Star vex GitHub repository to join the community',
			...
			print('   Future vex runs will now sync to the cloud.')
		else:
		...
		if task_id and sync_service:
			try:
				from vex.agent.cloud_events import UpdateAgentTaskEvent
...
@click.option('--output', '-o', type=click.Path(), help='Output file path for template (default: vex_<template>.py)')
...
def main(ctx: click.Context, debug: bool = False, **kwargs):
	...
	Examples:
	  uvx vex --template default
	  uvx vex --template advanced --output my_script.py
	"""
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from vex.mcp.server import main as mcp_main
...
def _run_template_generation(template: str, output: str | None, force: bool):
	...
	if output:
		...
		output_path = Path.cwd() / f'vex_{template}.py'
...
@click.option(
	...
	'-o',
	type=click.Path(),
	help='Output file path (default: vex_<template>.py)',
)
@click.option(
	'--force',
	'-f',
...
def init(
	...
	"""
	Generate a vex template file to get started quickly.
	...
	uvx vex init --template default

	\b
	# Generate advanced template with custom filename
	uvx vex init --template advanced --output my_script.py
