...
class ChatCompletionRequest(BaseModel):
    ...
    Extensions (non-OpenAI fields) are marked with 'x-vex'.
    """
    ...
    top_k: int = Field(20, ge = -1, le = 100, description = "[x-vex] Top-k sampling")
    min_p: float = Field(
        0.01, ge = 0.0, le = 1.0, description = "[x-vex] Min-p sampling threshold"
    )
    repetition_penalty: float = Field(
        1.1, ge = 1.0, le = 2.0, description = "[x-vex] Repetition penalty"
    )
    image_base64: Optional[str] = Field(
        None, description = "[x-vex] Base64-encoded image for vision models"
    )
    audio_base64: Optional[str] = Field(
        None, description = "[x-vex] Base64-encoded WAV for audio-input models (ASR)"
    )
    use_adapter: Optional[Union[bool, str]] = Field(
        None,
        description = (
            "[x-vex] Adapter control for compare mode. "
    ...
    enable_thinking: Optional[bool] = Field(
        None,
        description = "[x-vex] Enable/disable thinking/reasoning mode for supported models",
    )
    enable_tools: Optional[bool] = Field(
        None,
        description = "[x-vex] Enable tool calling for supported models",
    )
