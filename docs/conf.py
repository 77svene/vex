...
extensions = [
    ...
    "vexdocs",
    "sphinx.ext.autodoc",
    "vexfixautodoc",  # Must be after "sphinx.ext.autodoc"
    "sphinx.ext.coverage",
...
try:
    ...
    version = ".".join(map(str, vex.version_info[:2]))
    release = vex.__version__
except ImportError:
    version = ""
    release = ""
...
linkcheck_ignore = [
    r"http://localhost:\d+",
    "http://hg.vex.org",
    r"https://github.com/vex/vex/commit/\w+",
    r"https://github.com/vex/vex/issues/\d+",
]
...
coverage_ignore_pyobjects = [
    ...
    # settings.
    r"^vex\.downloadermiddlewares\.\w*?\.(\w*?Middleware|DownloaderStats)\.",
    # Base classes of downloader middlewares are implementation details that
    # are not meant for users.
    r"^vex\.downloadermiddlewares\.\w*?\.Base\w*?Middleware",
    ...
    r"^vex\.dupefilters\.[A-Z]\w*?\.(from_crawler|request_seen|open|close|log)$",
    # Private exception used by the command-line interface implementation.
    r"^vex\.exceptions\.UsageError",
    # Methods of BaseItemExporter subclasses are only documented in
    # BaseItemExporter.
    r"^vex\.exporters\.(?!BaseItemExporter\b)\w*?\.",
    ...
    r"^vex\.extensions\.[a-z]\w*?\.[A-Z]\w*?\.",  # methods
    r"^vex\.extensions\.[a-z]\w*?\.[a-z]",  # helper functions
    # Never documented before, and deprecated now.
    r"^vex\.linkextractors\.FilteringLinkExtractor$",
    # Implementation detail of LxmlLinkExtractor
    r"^vex\.linkextractors\.lxmlhtml\.LxmlParserLinkExtractor",
]
